...
- 执行层：Django management command 本地 worker
- 访问方式：启动服务的电脑作为本机服务，浏览器通过内网 IP 访问

当前只保留 React + Django + worker 的本地网络版，不包含桌面壳和 exe 打包。
...
| `npm run backend:worker` | 只启动本地 worker |
